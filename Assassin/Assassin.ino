...
String player_dir[3][2] = {{"52", "Ezio"}, {"84", "Altair"}, {"100", "Connor"}}; 
String player_id;
String player_name;
String killer_id;
boolean active_players[3] = {false, false, false};
int kills = 0;
int deaths = 0;
boolean stat_update = true;
long previousMillis = 0;
long update_interval = 4000;
...
void setup(){
  lcd.init(1,7,255,6,5,4,3,2,0,0,0,0);
  Serial.begin(19200);
  SPI.begin();
...
void loop(){
  ...
  if(buttonState == HIGH) {
    if(!isLcdLightOn) {
      digitalWrite(LCD_LIGHT_PIN, HIGH);
      Serial.println("Button pressed: screen is ON");
      lcd.display();
      if(stat_update) {
        stat_disp();
      }
      ...
      lcd.noDisplay();
      Serial.println("Button pressed: screen is OFF");
      isLcdLightOn = false;
    }
  }
  ...
  memcpy(serNum, str, 5);
  if (status == MI_OK && String(serNum[0]) != player_id)
  { 
    deaths++;
    killer_id = String(serNum[0]);
    if(isLcdLightOn) {
      stat_disp();
      stat_update = false;
    }
    ...
    String killer_str = "Killed by: ";
    killer_str += killer_id;
    Serial.println(killer_str);
    String input = "";
    input += "python send_kill.py kill ";
    input += killer_id;
    input += " ";
    input += player_id;
    char buff[input.length()+1];
    input.toCharArray(buff, sizeof(buff));
    Serial.println("buff = ");
    Serial.print(buff);
    system(buff);
    delay(200);
...
void lcdSetup() {
  ...
  lcd.home();
  lcd.cursor();
  Serial.println("Sign in screen...");
  lcd.print("Please tap in to");
  lcd.setCursor(0,1);
  ...
  while(!done) { 
    ...
    { 
      ...
      lcd.home();
      lcd.print("Welcome, ");
      for(int i = 0; i < sizeof(player_dir); i++) {
       if(player_dir[i][0] == String(serNum[0])) {
         active_players[i] = true;
         player_id = String(serNum[0]);
         player_name = player_dir[i][1];
         String id_str = "Player registered as: ";
         id_str += player_id;
         Serial.println(id_str);
         lcd.print(player_dir[i][1]);
         lcd.setCursor(0,1);
         lcd.print("Player# = ");
         lcd.print(serNum[0]);
         break;
  ...
  lcd.clear();
  lcd.home();
  Serial.println("Button screen...");
  lcd.print("Push button to");
  lcd.setCursor(0,1);
  lcd.print("join the action!");
  lcd.cursor();
  lcd.setCursor(15,1);
  lcd.blink();
  done = false;
  while(!done) {
    ...
    if(buttonState == HIGH) {
      ...
      lcd.noCursor();
      lcd.home();
      Serial.println("Instruction screen...");
      lcd.print("Use button to");
      lcd.setCursor(0,1);
      lcd.print("check stats...");
      for(x = 5; x >= 0; x--) {
        lcd.print(x);
        lcd.setCursor(14, 1);
        delay(750); 
      ...
      lcd.clear();
      lcd.home();
      Serial.println("Final screen...");
      lcd.print("Happy Hunting!");
      lcd.setCursor(0,1);
      lcd.print("LCD off in: ");
      for(x = 5; x >= 0; x--) {
        lcd.print(x);
        lcd.setCursor(12, 1);
        delay(750); 
...
void stat_disp() {
  String kills_str = "Kills: ";
  kills_str += kills;
  String deaths_str = "Deaths: ";
  deaths_str += deaths;
  Serial.println(kills_str);
  Serial.println(deaths_str);
  lcd.clear();
  lcd.home();
  lcd.print("Kills: ");
  lcd.print(kills);
...
void check_update() {
  ...
  system("cp /home/root/data /media/realroot/data");
  File dataF = SD.open("data");
  char input[10000];
  int index = 0;
  ...
  int z = s.indexOf(player_name);
  if(z >= 0) {
    String str = s.substring(z);
    int space_count = 0;
    int i = 0;
    while(space_count < 4) {
      if(str.charAt(i) == ' ') {
